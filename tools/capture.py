...
def main():
    ...
    # Resolved (and, if --check-out was given, validated) before the server
    # reachability check below so a bad --check-out is refused immediately —
    # no Chrome, no network, no dependency on the dev server being up.
    check_dir = resolve_check_out(args.check_out) if args.check else None
    # The confirming shots live INSIDE check_dir rather than in a temp root of
    # their own: check_dir is already the designated, outside-the-repo scratch
    # for a check, and this order is not about to mint a fresh os.tmpdir() tree
