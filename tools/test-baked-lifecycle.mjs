...
// the two silent catches at baked.js:88 (manifest fetch/parse) and :98
// (per-chapter .bin fetch), proven by real execution, not inspection.
//
...
async function run() {
  /* ---- SINGLETON — page-lifetime-singleton classification proof ---- */
  {
    ...
    fetchController.manifest = { version: 1, chapters: { owned: { file: 'owned.bin', sha256: 'x', keys: [], payload: { n: 7 } } } };
   ...
   *      DIFFERENT specifiers are genuinely independent modules.       */
  {
    resetFetch();
    fetchController.mode = 'manifest-ok';
    fetchController.manifest = { version: 1, chapters: { owned: { file: 'owned.bin', sha256: 'x', keys: [], payload: {} } } };
    fetchController.bins = { 'static/geom/owned.bin': new ArrayBuffer(8) };
  ...
  /* ---- baked.js:88 — manifest fetch throws (network/JSON error) ---- */
  {
    resetFetch();
    ...
    check(':88 catch — a manifest fetch that throws does NOT reject `ready` (absorbed, matches baked.js:88)', () =>
      A(rejected) === false);
    check(':88 catch — isBaked() stays false for the chapter that would have used this manifest', () =>
      A(baked.isBaked('owned')) === false);
  }

  /* ---- :88's positive-control contrast — a working manifest DOES    *
   *      flip isBaked() true, so the check above is not vacuous.      */
  {
    ...
    await baked.ready;
    check(':88 contrast — a working manifest+bin DOES flip isBaked() true (the :88 catch is reachable, not dead)', () =>
      A(baked.isBaked('owned')) === true);
  }

  /* ---- baked.js:86 — manifest fetch resolves but !res.ok (sibling   *
   *      early-return path into the same "stays false" outcome).      */
  {
    ...
    await baked.ready;
    check(':86 — a !res.ok manifest response also resolves `ready` cleanly, isBaked() false', () =>
      A(baked.isBaked('owned')) === false);
  }

  /* ---- baked.js:98 — per-chapter .bin fetch throws; per-chapter     *
   ...
   *      unaffected — matches the actual Promise.all shape).          */
  {
    ...
    await baked.ready;
    check(':98 catch — the sibling chapter whose .bin fetch succeeded IS baked (per-chapter isolation)', () =>
      A(baked.isBaked('owned')) === true);
    check(':98 catch — the chapter whose .bin fetch threw stays NOT baked, even though its manifest entry exists', () =>
      A(baked.isBaked('inspire')) === false);
  }

  /* ---- baked.js:96 — per-chapter .bin fetch resolves !res.ok         *
   *      (sibling early-return path into the same "stays false").     */
  {
    ...
    await baked.ready;
    check(':96 — a !res.ok .bin response also leaves isBaked() false', () =>
      A(baked.isBaked('owned')) === false);
  }

  /* ---- baked.js:91 — wrong manifest schema version (the guard the   *
   ...
   *      by ITS baseline).                                            */
  {
    ...
    await baked.ready;
    check(':91 — a wrong manifest.version leaves isBaked() false even though the bytes fetched fine', () =>
      A(baked.isBaked('owned')) === false);
  }

  /* ---- SOURCE-TEXT GUARD — mechanical proof this order's edits to   *
   *      baked.js are documentation-only: the exact original          *
   *      statements inside both catch bodies are still present,       *
   *      byte-for-byte, unchanged by the surrounding comments added.   *
   * ------------------------------------------------------------------ */
  {
    const src = readFileSync(BAKED_SRC, 'utf8');
    const originalReturn88 = 'return;                              // network/JSON error -> live everywhere';
    const originalComment98 = '/* absorbed: isBaked(id) stays false; that chapter builds live */';
    check('SOURCE GUARD — the original :88 return statement is present unchanged', () =>
      A(src.includes(originalReturn88)) === true);
    check('SOURCE GUARD — the original :98 catch-body comment is present unchanged', () =>
      A(src.includes(originalComment98)) === true);
  }
}
