...
//
//   tools/browser-smoke.mjs:674, :912, :1098, :1111, :1181 — the accepted
//   integrated forms sit inside strict `closePromise`,
//   `getWebglRendererString`, the SIGTERM/SIGKILL cleanup fallback in
//   `main()`'s `finally`, and the `isMain` IIFE — NONE of which
...
function run() {
  ...
  /* ---- §2: SOURCE GUARD — exact accepted integration statements at   *
   *      the unexported cleanup/diagnostic sites. The first is the      *
   *      strict owner-lifecycle replacement for removed closeWithin;    *
   *      the remainder preserve origin/main's catch behavior.          */
  {
    const originals = [
      [':674 — strict closePromise delegates to the owned resource close', 'return Promise.resolve().then(() => resource.close());'],
      [':912 — return null; inside getWebglRendererString\'s catch', 'return null;\n  } finally {\n    await page.close();'],
      [':1098 — process.kill(-server.pid, \'SIGTERM\') guarded by the try', "process.kill(-server.pid, 'SIGTERM');"],
      [':1098 fallback — server.kill(\'SIGTERM\') in the catch', "server.kill('SIGTERM');"],
      [':1111 — process.kill(-server.pid, \'SIGKILL\') guarded by the try', "process.kill(-server.pid, 'SIGKILL');"],
      [':1111 fallback — server.kill(\'SIGKILL\') in the catch', "server.kill('SIGKILL');"],
      [':1181 — the isMain comparison itself', 'return import.meta.url === pathToFileURL(process.argv[1]).href;'],
      [':1181 — the isMain catch fallback', 'return false;'],
    ];
