...
pin('A5', 'ONE INSTALL PER PAGE MODULE, on the REAL flags.js: three imports under one specifier record the five-op migration exactly ONCE, and a fourth under a query-busted specifier is a second module whose evaluation is the harmless two-op no-op A3 predicts',
  ...
  'the query-busted twin is not hypothetical: main.js imports organism/organism.js with a ?v= today. D1 is the census that says no module in this tree is named BOTH ways — this row is what makes that census worth having.');
...
if (PROVE) {
  ...
  const MUTANTS = [
    ...
    M('D1', 'a second importer names organism/organism.js WITHOUT the cache-buster, so the page evaluates that module twice and gets two of every module-global in it', ['cacheBusted', 'multiForm'],
      (i) => ({
        ...i,
        files: i.files.map(([rel, src]) => (rel === 'journey/journey.js'
          ? [rel, `import { createScene } from '../organism/organism.js';\n${src}`]
          : [rel, src])),
      })),
  ];
