...
// stream — the centre/source slot exactly AT it (cap az ~5.83, back-projected along
// BREEZE_DIR from the plume centred on world (3.24, 3.97, -0.50), the hero
// page's own 01-INSPIRE callout anchor), with two flank slots nearby
...
// edge and no portrait field could recover it (every candidate in an 8,640-
// point sweep failed). Arca therefore takes the DOWNWIND lip (6.98) where it
// projects leftmost, and the compact project chip takes the upwind one (4.68).
// The physical plumes are unchanged by that swap; only the labels move.
//   - Centre/source slot: FROZEN at 5.83 — it is the hero's own visible stream, and
...
//   - Right slot: 1.15 rad upwind, at the screen-RIGHT rim. Furthest upwind, so it
...
// Births are unaffected — every dot is still born in the centre/source wedge and
...
// pearls along each winding core read. The centre stream's is damped to 0.58
// because it already carries half the dots — without that the centre braid drowns the
...
// organism/spores.js, read-only), so the centre slot has 2,100 dots against
// the right slot's 924 and outshines both flanks however they are lit — the residual this file
...
//   centre     1.70-2.20 -> 1.55-2.35  mean 1.950 (spread x1.60: deliberately
//     DIFFUSED, so the centre stops drowning the flanks)
//   Arca       1.05-1.40 -> 1.13-1.32  mean 1.225 (spread x0.54: concentrated)
//   right      1.76-2.21 -> 1.87-2.10  mean 1.985 (spread x0.51: concentrated,
//     the weakest plume and the one that needed it most)
// D27 (2026-08-09) COUPLING: the wind condenses the ambient shed along these
// same three azimuths and rise maxima. Their one neutral source now lives at
// the repository root so organism/spores.js and this chapter cannot drift.
export { EXITS, EXIT_SLOT, exitIndexOf } from '../../../inspire-exits.js';
