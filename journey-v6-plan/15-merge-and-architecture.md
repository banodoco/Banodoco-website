...
| **M5 debt burn-down** | The catalogued leftovers: **delete the dead flight machinery in journeyState (all navigation is direct jumps now)**; frame-loop error isolation (one bad chapter can't freeze UI); scroll.js key-routing edge cases; hero `:focus-visible` styles + skip link; `?capture=` freeze param; **a no-self-ignition + locus audit of Connect/Owned/Final's on-screen transitions** (their occluded streaming is exempt; anything visible when it arms is not); a taste-dial registry (one pattern for TRANSFORM-class knobs); **retire the DOM input shield** — post-merge the organism is a library, so "controls off during the journey" becomes an input-policy mode the organism exposes, not an event interceptor taped over it (same reach-in class as buffer overwrites, aimed at input); **one flag registry** — today five files each parse `location.search` for their own flag (`?free ?aspect ?nosnap ?t ?tkdbg ?p ?g ?capture`) and storage keys carry the dev-era name (`journey-v6.transform`); all flags and keys move to one named module, keys renamed for the promoted site; **index.html becomes a shell** — it is 1,500 lines with two inline style blocks and two inline scripts (shield, fast-forward); each moves into the module that owns the behaviour, leaving the HTML as markup + one entry import; D11 wiring once decided; delete `assets/test-portraits/` before any public deploy (consent rule). | Each item checked off in EXECUTION.md |
