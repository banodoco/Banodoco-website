...
| # | Request | Notes |
|---|---|---|
| 72 | **Scrolling FORWARD off the last section cuts back to the first with a flicker.** She reads it as a bug, not a pacing complaint. Transcription partly garbled — "forward", "flicker" and "cut" are the only reliable words. | **Investigated; does not reproduce.** Eight forward wraps on the real wheel path, per presented frame, across four start states (deep link, end-hold `p=1.0` walked in by notches, gestured arrival, and the real page with the intro played): camera identical to 3 dp on the wrap frame, `eff` and `sky` worst single-frame jump **0.0000**, no chapter draw-gate toggle, DOM copy continuous, whole-frame luma worst single-frame step **2.53/255**. The destination-frame hypothesis and the new glide limiter are both ruled out with numbers. **Nothing changed.** Needs the rest of her sentence — whether the flicker is before, during or at the end of the movement — or a screen recording. `26-scroll-loop.md` §44–46. |
...
| Request | Outcome |
|---|---|
| 61/48/77/107 | **"Could you make the INDIVIDUAL MUSHROOMS AT THE END LIGHT UP SLOWER AND MORE STAGGERED"** — the fifth pass on this arrival, and the standing image is a town whose houses light one at a time. | **The stagger was never a tempo.** Four passes scaled the arrival uniformly (scrollVh 3.5->17.6, DRAW_W 0.16->0.50) and a uniform scale cannot touch a SHAPE: the 24-rung ladder is a 6.3x accelerando, so the last thirteen bodies kept arriving 21-45 ms apart however slow the whole thing got. The pull axis is exhausted (PULL_MAX pins the top), so the fix is time — and it only became reachable tonight, because `a0a89f8` armed the reveal limiter on commit glides and `TRANSIT_S` took scrollVh out of a glided arrival's pacing. Arrivals now get their own clock (`ARRIVE_GAP_S` 0.130 s per rung, `ARRIVE_RATE` 0.42 pull/s) while departures keep the shipped one, so `BAND_S`, `retireScale` and `bedSpread` are untouched. **Kindle x2.4-3.0, stagger x2.2-4.3, neither traded**; the head-to-tail collapse goes 5.5:1 -> 1.3:1 and the burst of six simultaneous arrivals becomes a flat one-to-three. No threshold, drawW, camera key or golden moves. 17-final-field.md, the 2026-08-14 (later) section. Residual: 375x812 keeps a ~200 ms tail burst, separately diagnosed. |
