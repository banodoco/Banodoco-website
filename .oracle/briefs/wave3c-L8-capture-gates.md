# Wave3C L8 — Capture, browser, and derived-artifact gate honesty

You are GPT-5.6 Luna, after Wave3B L1–L6, and must run this follow-up sequentially. Probe immutable base `1fa145fc51e89c8a1788db39aff98e775a576073` in `/Users/peteromalley/Documents/banodoco-workspace/banodoco-website-oracle-codebase-improvement`. Read S4, Wave3A gate output, and L1–L6 reports. Preserve source: no edits, implementation, installs, redesign/framework migration, baseline refresh, push/merge/deploy, or product-policy invention. Use disposable worktrees/temp directories and unique ports; isolate intentional output mutation. Report to stdout for capture with exact `path:line`, commands, exit/status matrix, diffs, environment, and binary classifications. Luna performs bounded exploration; Sol owns synthesis/judgment.
...
Run disposable command/exit cases for `tools/browser-smoke.mjs`, `tools/capture.py`, `tools/rebuild.py`, `tools/build-meta.py`, `tools/check.sh`, and release scripts: missing Chrome/launch failure, no WebGL, readiness timeout/no-journey, wrong frame/dimensions, known drift, full rebuild with changed mission/owned captures, and packaged-artifact smoke. Record authorized skip versus false pass, `git diff`, metadata hashes/order, and source-versus-package target identity. Confirm tracked `_check` mutation without leaving repository residue. Do not treat an unexecuted branch as a finding; label it untested.
