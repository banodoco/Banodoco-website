...
st.set_page_config(page_title="Banodoco - Animatediff")
...
st.header("Animatediff - building towards a controllable, beautiful and extendable video model")
...
st.write("Dall-E was the first time many saw coherent AI art. However, it wasn’t until Stable Diffusion 1.5 (SD) that most started seeing AI art that was legitimately aesthetically beautiful and artisticially interesting:")
...
st.write("In July 2023, [Animatediff](https://animatediff.github.io/) (AD) was released. It consists of a Motion Module - a model trained on the movement from millions of videos - that works in tandem with Stable Diffusion 1.5. AD determines the motion path a video should take, guiding images that are created by SD.")

         
st.write("While it’s currently limited and hard to control, so was SD when it first launched. Despite this, SD dramatically improved thanks to the open source community. I believe that AD may be SD for video - the beginning of an endless stream of improvements that result in controllable, beautiful AI video. While there have been other video approaches, I’ll cover why I think this is the right one for the community to build on top of - based on both evidence & wild speculation.")
...
st.markdown("### Which of these characteristics does Animatediff share?")

st.markdown("##### 1) Base model/approach that can deliver beautiful but limited results:")
st.write("Here are some examples of results from the base model:")
...
st.write("From our community alone, for example, B34STW4RS is building [fine-tuners to help others refine the model](https://github.com/B34STW4RS/AD-Evo-Tuner), while Kosinkadink [is building an extension on ComfyUI](https://github.com/Kosinkadink/ComfyUI-AnimateDiff) for others to experiment with the model. Many others are working on other elements while figuring out how to scale the approach.")
...
st.markdown("- Build stuff to **help others explore**: for example, as mentioned, many are already doing stuff and there are many other possibilities of ways to contribute - please join our Discord if you want to hear about collaboration opportunities.")
...
st.markdown("- Build orgs that **reward people who contribute w/ ownership/money**: will allow/incentivise more people to spend more time/effort contributing long-term.")


st.markdown("### Want to help our effort to keep the best version of this open source long-term?")

st.write("I spent the past year exploring and creating AI video - [making art](https://www.youtube.com/channel/UCIPL9xW3tdiK1eql4TuxW9Q), [building a tool](http://banodoco.ai/), experimenting with fine-tuning, etc. Like many, I fell in love - both with AI art & the potential of open-source to unlock this.  If you’re reading this, I’d guess that you did too.")

st.write("I believe open-source AI art is the best way to maximise the amount of creative fulfillment and beauty in the world. To support this, I want to build Banodoco into an org that can ship/supports cutting edge open-source video and supporting models. Additionally, I want us to build  the best product in the world for people who wish to access and combine these models to create beautiful things.")
