...
with footer1:  
  ...
  st.markdown("We're rethinking video creation from the ground up in an AI-native manner, building models and tools that aim to equip everyone with master-level artistic assistance.")
  st.markdown("Our goal is to help people who’ve dabbled in artistic creation with AI unlock the essence of creative expression—meticulously crafting work that is emotionally and intellectually impactful, helping others see the world from their unique perspective.")
...
st.subheader("Our philosophy: optimising for a satisfying, control-oriented creative process, while channeling the chaotic beauty of open source")
st.image("https://banodoco.s3.amazonaws.com/images/philosophy_1.webp",use_column_width='always')
...
st.write("Additionally, the process has to be fun and interactive so users stay engaged as they experiment and iterate towards the final result.")
...
st.write("Finally, we believe that channelling the chaos of open-source will be the optimal way to create the best tools & models - think of each model as a unique individual, waiting for someone to inspire them to become the best version of themselves. We believe that the ingenuity and creativity of the open source community can unlock this potential and maximise the amount of beauty in the world.")

st.markdown("***")

st.subheader("How we plan to deliver this")
st.image("https://banodoco.s3.amazonaws.com/images/philosophy_2.webp",use_column_width='always')
...
st.markdown("In addition to leveraging existing open models, we’ll create and build on top of a variety of models optimised for specific purposes - for example, <a href='/Steerable_Motion' target='_self'>Steerable Motion</a> and <a href='/Animatediff' target='_self'>Animatediff</a>.", unsafe_allow_html=True)
...
st.markdown("- We’ll create incentives for open source creators, apportioning the majority of our cash and equity to grants and rewards for open source hobbyists and researchers.")
st.markdown("- We'll open source every model and sub-model we create so others can build on top of them.")
st.markdown("- We’ll make it easy for people to run our product on their own GPUs for free - necessary to win over the open source community and hardcore artists.")
...
st.markdown("- **To continuously evolve with the changes to models:** Models will constantly change and evolve over the next decade. If we can align the incentives of OSS contributors, we can constantly evolve with these changes.")
