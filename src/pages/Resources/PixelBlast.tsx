...
type Uniforms = {
  ...
  uPatternOffset: { value: THREE.Vector2 };
};
...
void main(){
  ...
  float base = fbm2(uv + uPatternOffset, uTime * 0.05);
  base = base * 0.5 - 0.65;
...
const PixelBlast = forwardRef<PixelBlastHandle, PixelBlastProps>(({
  ...
  const prevConfigRef = useRef<PrevConfig | null>(null);

  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;
    speedRef.current = speed;
    const cfg: PrevConfig = { antialias, liquid, noiseAmount };
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(
      (entries) => {
        visibilityRef.current.visible = entries.some((e) => e.isIntersecting);
      },
      { threshold: 0 },
    );
  ...
  useImperativeHandle(ref, () => ({
    setPatternOffset: (x: number, y: number) => {
      const t = threeRef.current;
      if (t) t.uniforms.uPatternOffset.value.set(x, y);
    },
  }), []);
