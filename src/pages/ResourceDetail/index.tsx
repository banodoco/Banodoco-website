...
const ResourceDetail = () => {
  ...
  }, [navigate, resource, slug, username]);

  const displayMedia = activeMedia ?? galleryMedia[0] ?? null;
  const colorClass = resource ? (RESOURCE_TYPE_COLORS[resource.resourceType] ?? RESOURCE_TYPE_COLORS.other) : '';
  const creatorName = resource ? (resource.creator.displayName ?? resource.creator.username ?? 'Unknown') : '';

  return (
    <div className="bg-[#0b0b0f] text-zinc-100 min-h-screen">
      <div className="max-w-5xl mx-auto px-6 pt-20 pb-12 md:pt-24 md:pb-20">
        {/* Breadcrumb + creator */}
          ...
          {resource && (
            <div className="flex min-w-0 items-center gap-3">
              {resource.creator.avatarUrl ? (
                <img src={resource.creator.avatarUrl} alt={creatorName} className="w-8 h-8 rounded-full flex-shrink-0" loading="lazy" />
              ) : (
              ...
              {resource.creator.profileUrl ? (
                <Link to={resource.creator.profileUrl} className="max-w-[14rem] truncate text-sm font-medium text-zinc-300 hover:text-white transition">
                  {creatorName}
                </Link>
              ) : (
                <span className="max-w-[14rem] truncate text-sm font-medium text-zinc-300">{creatorName}</span>
              )}
            </div>
          )}
        </div>
        ...
        {error && !loading && (
          <div className="text-center py-20">
            ...
            <Link to="/resources" className="text-sm text-blue-400 hover:underline">Back to Resources</Link>
          </div>
        )}
        ...
        {resource && !loading && (
          ...
          >
              ...
              <div>
                {displayMedia.cloudflare_playback_hls_url ? (
                  ...
                  <div className="w-full aspect-video rounded-xl overflow-hidden bg-black">
                    <img src={displayMedia.cloudflare_thumbnail_url} alt={resource.title} className="w-full h-full object-contain" />
                  </div>
                ) : null}
                    ...
                    {galleryMedia.map(media => (
                      ...
                      >
                        {media.cloudflare_thumbnail_url ? (
                          <img src={media.cloudflare_thumbnail_url} alt="" className="w-full h-full object-cover" loading="lazy" />
                        ) : (
            ...
            {/* Linked art (media made with this asset) */}
            {linkedArt.length > 0 && (
              <div className="pt-6 border-t border-white/5">
                <h3 className="text-sm font-semibold text-zinc-400 uppercase tracking-wider mb-4">
                  Art made with this {resource.resourceType}
                </h3>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {linkedArt.map(art => (
                    <Link
                      key={art.id}
                      to={buildArtPath(art.id, art.title, art.creatorUsername)}
                      className="group rounded-lg overflow-hidden border border-white/5 hover:border-white/20 transition"
                    >
                      {art.thumbnailUrl ? (
                        <img
