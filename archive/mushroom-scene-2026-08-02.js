...
const TAP_ZETA = 0.14; // light damping: a few visible wobbles, settled in ~3s
{
  ...
  function shedSpores(n) {
    ...
    for (let k = 0; k < n; k++) {
      ...
      arr[i3]     = sporeOrigin[i3] * swayCos - sporeOrigin[i3 + 1] * swaySin;
      arr[i3 + 1] = sporeOrigin[i3] * swaySin + sporeOrigin[i3 + 1] * swayCos;
  ...
  renderer.domElement.addEventListener('pointerup', (e) => {
    ...
    if (hit) {
      ...
      if (hit.point.y > 2.8) shedSpores(28); // cap taps rattle a few spores loose
      tick();
      return;
    }
