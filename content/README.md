...
| ship **real portrait photos** | NOT deliverable via `content.js`. Every row ships `consent: false` (`content/content.js:629-630`, names are the literal string "Contributor" until the consent pipeline lands). The photo pipeline is QA-only behind `?photos=1` (`flags.js:105`), gated in code (`journey/chapters/owned/index.js:106-108` — "restore the unconditional promotion then, alongside the consent wiring"), and the current set is look-dev test portraits that must never ship (`assets/test-portraits/`, pre-deploy checklist `README.md:29-36`, `DEPLOY.md:61-62`). Real faces need: the consented asset set + `consent: true` wiring + the flag promotion + the Owned goldens re-shot (`tools/capture.py --pose owned`) |
