...
// =====================================================================
// TABLE OF CONTENTS (order as they appear below)
//   1. RNG / palette utils
...
//  10. Spore cloud        — shed from the back-side gill surface
...
const DRAW_GLSL = `
      ...
      float drawAt(vec3 p) {
        ...
        float lid = 1.0 - smoothstep(uClampY - 0.2, uClampY, wp.y) * (1.0 - step(1.5, uProg));
...
const _dbg = location.search.includes('dbg')
  ...
  : null;
addAnimator('breeze', (t) => {
  const b = breeze(t);
  const z = -b * 0.034;
  swayGroup.rotation.z = z;
  swayGroup.rotation.x =  b * 0.007; // a slight nod, so the sway isn't flat
  // the head trails the stalk: same signal, delayed, so the cap whips a beat late
  capBend.rotation.z = -breeze(t - 0.30) * 0.013;
...
addAnimator('highlights', (t) => _hlUpdate(t));
...
if (_introAt !== null) {
  ...
  addAnimator('intro-draw', () => {
    ...
    if (lived >= intro) {
      drawU.value = 2; // parked: fully drawn, stroke embers off
      shellsRestore();
